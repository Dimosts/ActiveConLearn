...
from cpmpy import boolvar, Model, all, sum, SolverLookup, intvar
...
def find_optimal_vars_subset(R, Y, kappaB, P_c):
    hashY = [hash(y) for y in Y]
    hashR = [hash(r) for r in R]

    x = boolvar(shape=(len(Y),))

    model = Model()

    constraints_Y1 = sum((1 - 10 * ((1 / P_c[i]) <= math.log2(len(Y)))) * all(hash(scope_var) in hashR or x[hashY.index(hash(scope_var))] for scope_var in get_scope(kappaB[i])) for i in
                         range(len(kappaB)))
...
def generate_findc2_query(L, delta):
    ...
    # run with the objective
    s.minimize(100 * p - p_soft_con[p])  # TODO: check if working properly
...
def generate_findc_query3(L, delta, P_c):
    # constraints from  B are taken into account as soft constraints who we do not want to satisfy (i.e. we want to violate)
    # This is the version of query generation for the FindC function that was presented in "Constraint acquisition via Partial Queries", IJCAI 2013

    tmp = Model(L)

    viol = sum([c for c in delta])  # get the amount of satisfied constraints from B
    ...
    # so a solution was found, try to find a better one now
    s.solution_hint(Y, values)
    V = boolvar(shape=(len(delta),))

    objective = sum(
        [~v * (1 - 2 * ((1 / P_c[c]) <= math.log2(len(Y))) * viol) for
         v, c in zip(V, range(len(delta)))])
